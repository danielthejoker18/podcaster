...
⚠️ Instructions:
- Do NOT include an "Introduction" or "Conclusion" section. Start directly with the first topic.
...
⚠️ Instruções:
- NÃO inclua uma seção de "Introdução" ou "Conclusão". Comece diretamente com o primeiro tópico.
...
"""
    ...
    "moderator": {
        "en": """You are {host_name}, the host of this podcast. Based on the section titles and the speakers, create:

1. An engaging introduction to start the episode, welcoming the listeners and introducing the guests ({speaker_names}).
2. Transition lines between each section.
3. A warm closing message to sign off.
...
⚠️ Instructions:
- Speak in the first person ("I", "We").
...
""",
        "pt": """Você é {host_name}, o apresentador deste podcast. Com base nos títulos das seções e nos participantes, crie:

1. Uma introdução envolvente para iniciar o episódio, dando as boas-vindas aos ouvintes e apresentando os convidados ({speaker_names}).
...
⚠️ Instruções:
- Fale na primeira pessoa ("Eu", "Nós").
